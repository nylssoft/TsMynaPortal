...
export class PasswordChangePage implements Page {
    ...
    private async renderEditAsync(parent: HTMLElement, pageContext: PageContext): Promise<void> {
        ...
        Controls.createLabel(divConfirmPwd, "newpwd-id", "form-label", pageContext.locale.translate("LABEL_CONFIRM_PWD"));
    ...
    private async onSaveAsync(e: Event, pageContext: PageContext): Promise<void> {
        ...
        try {
            ...
            if (!this.verifyPasswordStrength(newpwd.value)) {
    ...
    private verifyPasswordStrength(pwd: string): boolean {
        if (pwd.length >= 8) {
